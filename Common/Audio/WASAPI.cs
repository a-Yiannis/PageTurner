...
public static class WASAPI {
	...
	public static float GetSystemPeakValue() {
		...
		try {
			...
			if (enumerator.GetDefaultAudioEndpoint((int)EDataFlow.eRender, (int)ERole.eConsole, out device) != 0 ||
			    device is null) {
				return 0f;
			}
			...
			if (device.Activate(ref meterGuid, (int)CLSCTX.CLSCTX_ALL, IntPtr.Zero, out object meterObj) != 0 ||
			    meterObj is null) {
				return 0f;
			}
